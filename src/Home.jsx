...
const Home = () => {
  ...
  useEffect(() => {
    ...
    var particles = createParticles(80);
  ...
  const createParticles = (numParticles) => {
    const particles = [];
    const margin = 0.05 * canvasRef.current.width; // 5% margin from sides

    for (let i = 0; i < numParticles; i++) {
      let x = Math.random() * (canvasRef.current.width - margin * 2) + margin;
      let y = Math.random() * (canvasRef.current.height - margin * 2) + margin;
      ...
      if (y > canvasRef.current.height / 2) {
        y += margin;
      } else {
        y -= margin;
      }
